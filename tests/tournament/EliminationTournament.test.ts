...
function makeTeam(id: string, playerNames: string[] = [id]): TournamentTeam {
  return {
    id,
...
describe('EliminationTournament', () => {
  ...
  describe('totalRounds', () => {
    it('2 teams → 1 round', () => {
      const t = EliminationTournament.create().start(makeTeams(['A', 'B']), 2);
      expect(t.totalRounds()).toBe(1);
    });

    it('4 teams → 2 rounds', () => {
      const t = EliminationTournament.create().start(makeTeams(['A', 'B', 'C', 'D']), 2);
      expect(t.totalRounds()).toBe(2);
    });

    it('8 teams → 3 rounds', () => {
      const t = EliminationTournament.create().start(makeTeams(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']), 2);
  ...
  describe('wbR1Losers', () => {
    it('returns empty before any results', () => {
      const t = EliminationTournament.create().start(makeTeams(['A', 'B', 'C', 'D']), 4);
  ...
  describe('validate', () => {
    it('requires at least 2 teams', () => {
      ...
      expect(t.validate([makeTeam('a')], 'singles')).not.toBeNull();
    });

    it('accepts 2 or more teams for singles', () => {
      const t = EliminationTournament.create();
      expect(t.validate(makeTeams(['A', 'B']), 'singles')).toBeNull();
    });
